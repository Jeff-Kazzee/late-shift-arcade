...
| **DS-1b — stress** | Streaming/edit/light/remesh under churn, **desktop-web first (owner's word 2026-07-24: web-first; device testing deferred)**: worker-based meshing behind immutable mesh-job snapshots with one-voxel halos, revision-stamped cancellation and stale rejection (F-06); measured — not asserted — desktop budget numbers (F-05) | Desktop §13.5 column measured under adversarial-geometry fixtures (caves, checkerboards, player builds) at radius 8; the mobile column and shelf-device capture move to the DS-5 release gate |
